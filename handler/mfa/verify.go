...
func (h *MFAHandler) loadMFAVerifyContext(c fiber.Ctx) (*mfaVerifyRequest, string, error) {
	...
	if req.MFAToken == "" {
		return nil, "", response.JSONError(fiber.StatusBadRequest, "mfa_token is required")
	}

	userID, err := h.cache.Get(c.Context(), "mfa_pending:"+req.MFAToken)
	if err != nil || userID == "" {
		return nil, "", response.JSONError(fiber.StatusUnauthorized, "invalid or expired mfa_token")
	}
	...
	if h.isMFAVerifyLocked(c.Context(), req.MFAToken) {
		return nil, "", response.JSONError(fiber.StatusTooManyRequests, "too many MFA attempts, please log in again")
	}
...
func (h *MFAHandler) verifyMFACode(c fiber.Ctx, req *mfaVerifyRequest, userID string, cfg *model.MFAConfig) error {
	...
	if locked {
		...
		return response.JSONError(fiber.StatusTooManyRequests, "too many MFA attempts, please log in again")
	}
	return response.JSONError(fiber.StatusUnauthorized, "invalid MFA code")
}
